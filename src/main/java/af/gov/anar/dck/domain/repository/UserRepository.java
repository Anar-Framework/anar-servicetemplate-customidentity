...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT new af.gov.anar.dck.domain.model.User(u.id, u.name, u.address, u.username, u.active, u.odkPassword, u.email, u.avatar, u.createdAt, u.updatedAt) from User u INNER JOIN u.environments e where e.slug=?1 order by u.id")
    ...
    @Query("SELECT new af.gov.anar.dck.domain.model.Group(g.id, g.name, g.description, g.active, g.envSlug) from User u INNER JOIN u.groups g where u.id = ?1 and g.envSlug = ?2")
