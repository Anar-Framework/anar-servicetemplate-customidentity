...
public class TableConstants {
    ...
    /**
     * ID is the primary key, as experienced by the user. There may be multiple rows
     * with this ID due to conflicts and checkpoint states.
     */
    public static final String ID = "_id";

    /**
     * ROW_ETAG defines the the server entity tag (version) of the row. This is
     * controlled by the server and changes by the client are always ignored.
     */
     ...
     * controls on this row. Changes to this value on the device may or may not be
     * applied during a sync action (TBD).
     */
    ...
    /**
     * (form_id, locale, savepoint_type, savepoint_timestamp, savepoint_creator) are
     * the tuple written and managed by ODK Survey when a record is updated.
     *
     * ODK Tables needs to update these appropriately when a cell is directly edited
     * based upon whether or not the table is 'form-managed' or not. If
    ...
    /**
     * This set contains the names of all the metadata columns that are specific to
     * each phone and whose data should NOT be synched onto the server and between
     * phones.
     */
    ...
    static {
        ...
        CLIENT_ONLY_COLUMN_NAMES.add(ID); // somewhat of a misnomer -- this is transmitted and serves as a PK to the
                                          // record.
        CLIENT_ONLY_COLUMN_NAMES.add(ROW_ETAG); // somewhat of a misnomer -- this is transmitted, but never overwrites
                                                // server.
    ...
    /**
     * Converts an unix timestamp to a nanosecond-extended iso8601-style UTC date
     * yyyy-mm-ddTHH:MM:SS.sssssssss. When {@code locale} is null, the system
    ...
    public static String nanoSecondsFromMillis(Long timeMillis, Locale locale) {
        ...
        // convert to a nanosecond-extended iso8601-style UTC date
    ...
    /**
     * Converts a nanosecond-extended iso8601-style UTC date
     * yyyy-mm-ddTHH:MM:SS.sssssssss to an unix timestamp. When {@code locale} is
    ...
    public static Long milliSecondsFromNanos(String timeNanos, Locale locale) {
        ...
        // convert from a nanosecond-extended iso8601-style UTC date
        // yyyy-mm-ddTHH:MM:SS.sssssssss
        Date d;
        try {
